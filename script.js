...
class MarkdownLoader {
    constructor() {
        this.sections = ['about', 'news', 'projects', 'collaborations', 'resume'];
        this.init();
    }
        ...
        for (const fullPath of pathsToTry) {
            try {
                ...
                if (response.ok) {
                    ...
                    // Initialize project filter after projects section loads
                    if (section === 'projects' && typeof window.initProjectFilter === 'function') {
...
// Project filter — initialized after MarkdownLoader injects projects.md content
class ProjectFilter {
    constructor() {
        this.activeFilters = new Set();
        window.initProjectFilter = () => this.init();
    }
    ...
    applyFilter() {
        const cards = document.querySelectorAll('#projects-content .publication-card');
        cards.forEach(card => {
            if (this.activeFilters.size === 0) {
                card.style.display = '';
                return;
            ...
            const matches = [...this.activeFilters].some(f => cardTags.includes(f));
